// Proxy route for gedung data from external SIAKAD service
...
function buildTargetUrl(request: Request | NextRequest): string {
  ...
  return `${SIAKAD_URL.replace(/\/$/, "")}/api/v1/ref/gedung${qs}`;
}

async function fetchExternalGedung(url: string): Promise<GedungResponse> {
...
export async function GET(request: Request | NextRequest) {
  try {
    ...
    const data = await fetchExternalGedung(target);

    return NextResponse.json(data);
  } catch (err) {
    console.error("/api/data-kampus/gedung error:", err);
