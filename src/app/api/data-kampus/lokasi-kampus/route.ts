// Proxy route for lokasi kampus data from external SIAKAD service
...
function buildTargetUrl(request: Request | NextRequest): string {
  ...
  return `${SIAKAD_URL.replace(/\/$/, "")}/api/v1/ref/lokasi-kampus${qs}`;
}

async function fetchExternalLokasiKampus(url: string): Promise<LokasiKampusResponse> {
  ...
  return data as LokasiKampusResponse;
}
...
export async function GET(request: Request | NextRequest) {
  try {
    ...
    const data = await fetchExternalLokasiKampus(target);

    return NextResponse.json(data);
  } catch (err) {
    console.error("/api/data-kampus/lokasi-kampus error:", err);
