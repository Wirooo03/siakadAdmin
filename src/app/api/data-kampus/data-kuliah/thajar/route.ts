// Proxy route for tahun ajaran (ref) from external service
...
function buildTargetUrl(request: Request | NextRequest): string {
  ...
  return `${SIAKAD_URL.replace(/\/$/, "")}/api/v1/ref/tahun-ajaran${qs}`;
}

async function fetchExternalTahunAjar(url: string): Promise<TahunAjarResponse> {
  ...
  return data as TahunAjarResponse;
}
...
export async function GET(request: Request | NextRequest) {
  try {
    ...
    const data = await fetchExternalTahunAjar(target);

    return NextResponse.json(data);
  } catch (err: any) {
    console.error("/api/data-kampus/perkuliahan/thajar error:", err);
