// Proxy route for Jenis Mata Kuliah (jenis-matkul) reference data
...
function buildTargetUrl(request: Request | NextRequest): string {
	...
	return `${SIAKAD_URL.replace(/\/$/, "")}/api/v1/ref/jenis-matkul${qs}`;
}

async function fetchExternalJenisMk(url: string): Promise<JenisMkResponse> {
	...
	return data as JenisMkResponse;
}
...
export async function GET(request: Request | NextRequest) {
	try {
		...
		const data = await fetchExternalJenisMk(target);

		return NextResponse.json(data);
	} catch (err: any) {
		console.error("/api/data-kampus/data-kuliah/jenismk error:", err);
