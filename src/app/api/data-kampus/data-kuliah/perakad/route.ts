...
// Proxy route for Periode Akademik (periode-akademik)
...
function buildTargetUrl(request: Request | NextRequest): string {
	...
	return `${SIAKAD_URL.replace(/\/$/, "")}/api/v1/periode-akademik${qs}`;
}

async function fetchExternalPerakad(url: string): Promise<PerakadResponse> {
	...
	return data as PerakadResponse;
}
...
export async function GET(request: Request | NextRequest) {
	try {
		...
		const data = await fetchExternalPerakad(target);

		return NextResponse.json(data);
	} catch (err: any) {
		console.error("/api/data-kampus/data-kuliah/perakad error:", err);
