// Proxy route for Tahun Kurikulum data from external service
...
function buildTargetUrl(request: Request | NextRequest): string {
  ...
  return `${SIAKAD_URL.replace(/\/$/, "")}/api/v1/tahun-kurikulum${qs}`;
}

async function fetchExternalTahunKurikulum(url: string): Promise<TahunKurikulumResponse> {
  ...
  return data as TahunKurikulumResponse;
}
...
export async function GET(request: Request | NextRequest) {
  try {
    ...
    const data = await fetchExternalTahunKurikulum(target);

    return NextResponse.json(data);
  } catch (err: any) {
    console.error("/api/data-kampus/(perkuliahan)/thkur error:", err);
