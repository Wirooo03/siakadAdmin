...
function buildTargetUrl(request: Request | NextRequest): string {
  ...
  return `${SIAKAD_URL.replace(/\/$/, "")}/api/v1/ref/jenjang-pendidikan${qs}`;
}

async function fetchExternalJenjangPendidikan(url: string): Promise<JenjangPendidikanResponse> {
  ...
  return data as JenjangPendidikanResponse;
}
...
export async function GET(request: Request | NextRequest) {
  try {
    ...
    const data = await fetchExternalJenjangPendidikan(target);

    return NextResponse.json(data);
  } catch (err) {
    console.error("/api/perkuliahan/jenjang-pendidikan error:", err);
