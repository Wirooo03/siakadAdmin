// Proxy route for kurikulum prodi data from external service
...
function buildTargetUrl(request: Request | NextRequest): string {
  ...
  return `${SIAKAD_URL.replace(/\/$/, "")}/api/v1/kurikulum-prodi${qs}`;
}

async function fetchExternalKurikulumProdi(url: string): Promise<KurikulumProdiResponse> {
  ...
  return data as KurikulumProdiResponse;
}
...
export async function GET(request: Request | NextRequest) {
  try {
    ...
    const data = await fetchExternalKurikulumProdi(target);

    return NextResponse.json(data);
  } catch (err: any) {
    console.error("/api/perkuliahan/kurprod error:", err);
