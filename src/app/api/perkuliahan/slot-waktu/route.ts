...
function buildTargetUrl(request: Request | NextRequest): string {
  ...
  return `${SIAKAD_URL.replace(/\/$/, "")}/api/v1/ref/slot-waktu${qs}`;
}

async function fetchExternalSlotWaktu(url: string): Promise<SlotWaktuResponse> {
  ...
  return data as SlotWaktuResponse;
}
...
export async function GET(request: Request | NextRequest) {
  try {
    ...
    const data = await fetchExternalSlotWaktu(target);

    return NextResponse.json(data);
  } catch (err) {
    console.error("/api/perkuliahan/slot-waktu error:", err);
