...
export default function Page() {
	...
	const { data, isLoading } = useRuanganTable({ page, per_page: perPage, q });

	// Debug logs
	console.log('Ruangan data:', data);
	...
	return (
		<div className="min-h-screen bg-slate-50 p-4 md:p-8 overflow-hidden">
			...
			<div className="mb-6 md:mb-8 slide-in-left">
				...
				<p className="mt-1 text-sm md:text-base text-slate-600 hover:text-slate-800 transition-colors duration-200">Kelola data ruangan kuliah</p>
			</div>
				...
				<div className="flex flex-col sm:flex-row flex-wrap items-stretch sm:items-center gap-3 md:gap-4">
					...
					<div className="flex items-center gap-2 md:gap-3 group">
						...
						<div className="ml-auto">
							<Link
								href="/siakad/admin/create-ruangan"
							...
							<tr className="bg-indigo-600 text-white">
								...
								<th className="px-3 md:px-6 py-3 md:py-4 text-left text-xs md:text-sm font-semibold tracking-wider group hover:bg-indigo-700 transition-colors duration-200">
									<div className="flex items-center gap-1 md:gap-2">
										...
										<span className="group-hover:translate-x-1 transition-transform duration-200">Nama Ruangan</span>
									</div>
								</th>
								<th className="px-3 md:px-6 py-3 md:py-4 text-left text-xs md:text-sm font-semibold tracking-wider group hover:bg-indigo-700 transition-colors duration-200">
									<div className="flex items-center gap-1 md:gap-2">
										...
										<span className="group-hover:translate-x-1 transition-transform duration-200">Lokasi</span>
									</div>
								</th>
								<th className="px-3 md:px-6 py-3 md:py-4 text-left text-xs md:text-sm font-semibold tracking-wider group">
									<div className="flex items-center gap-1 md:gap-2">
										...
										<span className="group-hover:translate-x-1 transition-transform duration-200">Unit</span>
									</div>
								</th>
								<th className="px-3 md:px-6 py-3 md:py-4 text-left text-xs md:text-sm font-semibold tracking-wider group hover:bg-indigo-700 transition-colors duration-200">
									<div className="flex items-center gap-1 md:gap-2">
								...
								<tr>
									<td colSpan={7} className="px-6 py-12 text-center">
										<div className="flex flex-col items-center justify-center gap-3">
											...
											<p className="text-slate-500 font-medium">Tidak ada data ruangan</p>
										</div>
									</td>
								</tr>
							) : (
								data.data.data.map((row: RuanganItem, idx: number) => (
									<tr
										key={row.kode_ruang}
									...
									>
										...
										<td className="px-3 md:px-6 py-3 md:py-4">
											<span className={`inline-flex items-center gap-1 px-2 md:px-3 py-1 rounded-full text-[10px] md:text-xs font-semibold transition-all duration-300 hover:scale-110 hover:shadow-md ${
												...
												{row.aktif ? (
													<><FiCheckCircle className="text-xs" /> Aktif</>
												) : (
													<><FiXCircle className="text-xs" /> Nonaktif</>
												)}
											</span>
										</td>
										<td className="px-3 md:px-6 py-3 md:py-4 text-right">
											<div className="inline-flex items-center gap-2">
												<Link
													href={`/siakad/admin/ruangan/${row.kode_ruang}`}
												...
												<button
													onClick={() => {
														if (confirm('Hapus ruangan ini?')) {
															console.log('delete', row.kode_ruang);
					...
					<div className="px-3 md:px-6 py-4 md:py-5 bg-slate-50 border-t border-slate-200">
						<div className="flex flex-col sm:flex-row flex-wrap items-center justify-between gap-3 md:gap-4">
							<div className="text-xs md:text-sm text-slate-600 font-medium hover:text-indigo-600 transition-colors duration-200 cursor-default text-center sm:text-left">
								Halaman <span className="text-indigo-600 font-bold px-1.5 md:px-2 py-0.5 md:py-1 bg-indigo-50 rounded-lg hover:bg-indigo-100 hover:scale-110 inline-block transition-all duration-200">{data.data.current_page}</span> dari{" "}
								<span className="text-indigo-600 font-bold px-1.5 md:px-2 py-0.5 md:py-1 bg-indigo-50 rounded-lg hover:bg-indigo-100 hover:scale-110 inline-block transition-all duration-200">{data.data.last_page}</span> — Total:{" "}
								<span className="text-indigo-600 font-bold px-1.5 md:px-2 py-0.5 md:py-1 bg-indigo-50 rounded-lg hover:bg-indigo-100 hover:scale-110 inline-block transition-all duration-200">{data.data.total}</span> data
							</div>
							...
							<div className="flex items-center gap-1.5 md:gap-2">
								...
								{Array.from({ length: data.data.last_page }, (_, i) => i + 1)
									.slice(Math.max(0, page - 2), Math.min(data.data.last_page, page + 1))
									.map((p) => (
										<button
											key={p}
											onClick={() => setPage(p)}
