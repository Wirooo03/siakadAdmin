...
import { usePerakadTable } from "@/lib/hooks/data-kampus/data-kuliah/perakad/usePerakadTable";
import type { PerakadItem } from "@/lib/services/data-kampus/data-kuliah/perakad/type";
...
export default function Page() {
	...
	const { data, isLoading, error } = usePerakadTable({ page, per_page: perPage, q });
	...
	return (
		<div className="min-h-screen bg-slate-50 p-4 md:p-8 overflow-hidden">
			...
			<div className="mb-6 md:mb-8 slide-in-left">
				...
				<p className="mt-1 text-sm md:text-base text-slate-600 hover:text-slate-800 transition-colors duration-200">Kelola data periode akademik</p>
			</div>
				...
				<div className="flex flex-col sm:flex-row flex-wrap items-stretch sm:items-center gap-3 md:gap-4">
					...
					<div className="flex items-center gap-2 md:gap-3 group">
						...
						<div className="ml-auto">
							<Link
								href="/siakad/admin/create-perakad"
					...
					<thead>
							<tr className="bg-indigo-600 text-white">
								...
								<th className="px-3 md:px-6 py-3 md:py-4 text-left text-xs md:text-sm font-semibold tracking-wider group hover:bg-indigo-700 transition-colors duration-200">
									<div className="flex items-center gap-1 md:gap-2">
										...
										<span className="group-hover:translate-x-1 transition-transform duration-200">Nama Periode</span>
									</div>
								</th>
								<th className="px-3 md:px-6 py-3 md:py-4 text-left text-xs md:text-sm font-semibold tracking-wider group hover:bg-indigo-700 transition-colors duration-200">
									<div className="flex items-center gap-1 md:gap-2">
										...
										<span className="group-hover:translate-x-1 transition-transform duration-200">Tgl Awal Kuliah</span>
									</div>
								</th>
								<th className="px-3 md:px-6 py-3 md:py-4 text-left text-xs md:text-sm font-semibold tracking-wider group">
									<div className="flex items-center gap-1 md:gap-2">
										...
										<span className="group-hover:translate-x-1 transition-transform duration-200">Tgl Akhir Kuliah</span>
									</div>
								</th>
								<th className="px-3 md:px-6 py-3 md:py-4 text-left text-xs md:text-sm font-semibold tracking-wider group hover:bg-indigo-700 transition-colors duration-200">
									<div className="flex items-center gap-1 md:gap-2">
										<FiCalendar className="text-indigo-200 group-hover:text-white group-hover:scale-125 transition-all duration-300 text-sm md:text-base" />
										<span className="group-hover:translate-x-1 transition-transform duration-200">Tgl Awal UTS</span>
									</div>
								</th>
								<th className="px-3 md:px-6 py-3 md:py-4 text-left text-xs md:text-sm font-semibold tracking-wider group hover:bg-indigo-700 transition-colors duration-200">
									<div className="flex items-center gap-1 md:gap-2">
										<FiCheckCircle className="text-indigo-200 group-hover:text-white group-hover:scale-125 transition-all duration-300 text-sm md:text-base" />
						...
						<tbody className="divide-y divide-slate-100">
							{isLoading ? (
								...
								data?.data.map((row: PerakadItem, idx: number) => (
									<tr
										key={row.kode}
									...
									>
										...
										<td className="px-3 md:px-6 py-3 md:py-4">
											<span className={`inline-flex items-center gap-1 px-2 md:px-3 py-1 rounded-full text-[10px] md:text-xs font-semibold transition-all duration-300 hover:scale-110 hover:shadow-md ${
												...
												{row.aktif === 1 ? 'Aktif' : 'Tidak Aktif'}
											</span>
										</td>
										<td className="px-3 md:px-6 py-3 md:py-4 text-right">
											<div className="inline-flex items-center gap-2">
												<Link
													href={`/siakad/admin/perakad/${row.kode}`}
												...
												<button
													onClick={() => {
														if (confirm('Hapus periode akademik ini?')) {
					...
					<div className="px-3 md:px-6 py-4 md:py-5 bg-slate-50 border-t border-slate-200">
						<div className="flex flex-col sm:flex-row flex-wrap items-center justify-between gap-3 md:gap-4">
							<div className="text-xs md:text-sm text-slate-600 font-medium hover:text-indigo-600 transition-colors duration-200 cursor-default text-center sm:text-left">
								...
								<span className="text-indigo-600 font-bold px-1.5 md:px-2 py-0.5 md:py-1 bg-indigo-50 rounded-lg hover:bg-indigo-100 hover:scale-110 inline-block transition-all duration-200">{Math.ceil(data.pagination.total / data.pagination.limit)}</span> — Total:{" "}
							...
							<div className="flex items-center gap-1.5 md:gap-2">
								...
								{/* Next Button */}
								<button
									onClick={() => setPage((p) => Math.min(Math.ceil(data.pagination.total / data.pagination.limit), p + 1))}
