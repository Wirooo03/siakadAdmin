...
import { useThajarTable } from "@/lib/hooks/data-kampus/data-kuliah/thajar/useThajarTable";
import type { TahunAjarItem } from "@/lib/services/data-kampus/data-kuliah/thajar/type";
...
export default function Page() {
	...
	const { data, isLoading, error } = useThajarTable({ page, per_page: perPage, q });
	...
	return (
		<div className="min-h-screen bg-slate-50 p-4 md:p-8 overflow-hidden">
			...
			<div className="mb-6 md:mb-8 slide-in-left">
				...
				<p className="mt-1 text-sm md:text-base text-slate-600 hover:text-slate-800 transition-colors duration-200">Kelola data tahun ajaran</p>
			</div>
				...
				<div className="flex flex-col sm:flex-row flex-wrap items-stretch sm:items-center gap-3 md:gap-4">
					...
					<div className="flex items-center gap-2 md:gap-3 group">
						...
						<div className="ml-auto">
							<Link
								href="/siakad/admin/create-thajar"
					...
					<thead>
							<tr className="bg-indigo-600 text-white">
								...
								<th className="px-3 md:px-6 py-3 md:py-4 text-left text-xs md:text-sm font-semibold tracking-wider group hover:bg-indigo-700 transition-colors duration-200">
									<div className="flex items-center gap-1 md:gap-2">
										...
										<span className="group-hover:translate-x-1 transition-transform duration-200">Nama Tahun Ajaran</span>
									</div>
								</th>
								<th className="px-3 md:px-6 py-3 md:py-4 text-left text-xs md:text-sm font-semibold tracking-wider group hover:bg-indigo-700 transition-colors duration-200">
									<div className="flex items-center gap-1 md:gap-2">
										<FiCheckCircle className="text-indigo-200 group-hover:text-white group-hover:rotate-12 group-hover:scale-125 transition-all duration-300 text-sm md:text-base" />
								...
								<th className="px-3 md:px-6 py-3 md:py-4 text-left text-xs md:text-sm font-semibold tracking-wider group">
									<div className="flex items-center gap-1 md:gap-2">
										...
										<span className="group-hover:translate-x-1 transition-transform duration-200">Tanggal Mulai</span>
									</div>
								</th>
								<th className="px-3 md:px-6 py-3 md:py-4 text-left text-xs md:text-sm font-semibold tracking-wider group hover:bg-indigo-700 transition-colors duration-200">
									<div className="flex items-center gap-1 md:gap-2">
										<FiClock className="text-indigo-200 group-hover:text-white group-hover:scale-125 transition-all duration-300 text-sm md:text-base" />
										<span className="group-hover:translate-x-1 transition-transform duration-200">Tanggal Selesai</span>
									</div>
								</th>
								<th className="px-3 md:px-6 py-3 md:py-4 text-left text-xs md:text-sm font-semibold tracking-wider group">
									<div className="flex items-center gap-1 md:gap-2">
						...
						<tbody className="divide-y divide-slate-100">
							{isLoading ? (
								...
								data?.data.map((row: TahunAjarItem, idx: number) => (
									<tr
										key={row.id_tahun_ajaran}
									...
									>
										...
										<td className="px-3 md:px-6 py-3 md:py-4">
											<span className={`inline-flex items-center gap-1 px-2 md:px-3 py-1 rounded-full text-[10px] md:text-xs font-semibold transition-all duration-300 hover:scale-110 hover:shadow-md ${
												row.a_aktif
												...
												{row.a_aktif ? <FiCheckCircle className="text-xs" /> : <FiXCircle className="text-xs" />}
												{row.a_aktif ? 'Aktif' : 'Tidak Aktif'}
											</span>
										</td>
										<td className="px-3 md:px-6 py-3 md:py-4 text-xs md:text-sm text-slate-600 group-hover:text-slate-800 transition-colors duration-200">
											{row.tgl_mulai ? new Date(row.tgl_mulai).toLocaleDateString('id-ID') : '-'}
										</td>
										<td className="px-3 md:px-6 py-3 md:py-4 text-xs md:text-sm text-slate-600 group-hover:text-slate-800 transition-colors duration-200">
											{row.tgl_selesai ? new Date(row.tgl_selesai).toLocaleDateString('id-ID') : '-'}
										</td>
										<td className="px-3 md:px-6 py-3 md:py-4 text-xs md:text-sm text-slate-600 group-hover:text-slate-800 transition-colors duration-200">
											{row.tahun}
										</td>
										<td className="px-3 md:px-6 py-3 md:py-4 text-right">
											<div className="inline-flex items-center gap-2">
												<Link
													href={`/siakad/admin/thajar/${row.id_tahun_ajaran}`}
												...
												<button
													onClick={() => {
														if (confirm('Hapus tahun ajaran ini?')) {
															console.log('delete', row.id_tahun_ajaran);
					...
					<div className="px-3 md:px-6 py-4 md:py-5 bg-slate-50 border-t border-slate-200">
						<div className="flex flex-col sm:flex-row flex-wrap items-center justify-between gap-3 md:gap-4">
							<div className="text-xs md:text-sm text-slate-600 font-medium hover:text-indigo-600 transition-colors duration-200 cursor-default text-center sm:text-left">
								Halaman <span className="text-indigo-600 font-bold px-1.5 md:px-2 py-0.5 md:py-1 bg-indigo-50 rounded-lg hover:bg-indigo-100 hover:scale-110 inline-block transition-all duration-200">{data.meta.current_page}</span> dari{" "}
								<span className="text-indigo-600 font-bold px-1.5 md:px-2 py-0.5 md:py-1 bg-indigo-50 rounded-lg hover:bg-indigo-100 hover:scale-110 inline-block transition-all duration-200">{data.meta.last_page}</span> — Total:{" "}
								<span className="text-indigo-600 font-bold px-1.5 md:px-2 py-0.5 md:py-1 bg-indigo-50 rounded-lg hover:bg-indigo-100 hover:scale-110 inline-block transition-all duration-200">{data.meta.total}</span> data
							</div>
							...
							<div className="flex items-center gap-1.5 md:gap-2">
								...
								{Array.from({ length: data.meta.last_page }, (_, i) => i + 1)
									.slice(Math.max(0, page - 2), Math.min(data.meta.last_page, page + 1))
									.map((p) => (
										<button
											key={p}
											onClick={() => setPage(p)}
								...
								{/* Next Button */}
								<button
									onClick={() => setPage((p) => Math.min(data.meta?.last_page || 1, p + 1))}
