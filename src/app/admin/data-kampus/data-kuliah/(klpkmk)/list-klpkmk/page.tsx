...
import { useKlpkmkTable } from "@/lib/hooks/data-kampus/data-kuliah/klpkmk/useKlpkmkTable";
import type { KelompokMkItem } from "@/lib/services/data-kampus/data-kuliah/klpkmk/type";
...
export default function Page() {
	...
	const { data, isLoading, error } = useKlpkmkTable({ q });

	// Filter data client-side based on search query
	const filteredData = data?.data?.filter((item: KelompokMkItem) => {
		if (!q) return true;
		const searchLower = q.toLowerCase();
		return (
			(item.id_kelompok_mk || '').toLowerCase().includes(searchLower) ||
			(item.nm_kelompok_mk || '').toLowerCase().includes(searchLower) ||
	...
	return (
		<div className="min-h-screen bg-slate-50 p-4 md:p-8 overflow-hidden">
			...
			<div className="mb-6 md:mb-8 slide-in-left">
				...
				<p className="mt-1 text-sm md:text-base text-slate-600 hover:text-slate-800 transition-colors duration-200">Kelola data kelompok mata kuliah</p>
			</div>
			...
			<div className="mb-4 md:mb-6 bg-white rounded-2xl shadow-xl p-4 md:p-6 border border-slate-100 scale-in slide-up-hover [animation-delay:0.1s]">
				<div className="flex flex-col sm:flex-row flex-wrap items-stretch sm:items-center gap-3 md:gap-4">
					...
					<div className="ml-auto">
						<Link
							href="/siakad/admin/create-klpkmk"
				...
				<table className="min-w-full divide-y divide-slate-100">
						...
						<tr className="bg-indigo-600 text-white">
							...
							<th className="px-3 md:px-6 py-3 md:py-4 text-left text-xs md:text-sm font-semibold tracking-wider group hover:bg-indigo-700 transition-colors duration-200">
								<div className="flex items-center gap-1 md:gap-2">
									...
									<span className="group-hover:translate-x-1 transition-transform duration-200">Nama Kelompok Mata Kuliah</span>
								</div>
							</th>
							<th className="px-3 md:px-6 py-3 md:py-4 text-left text-xs md:text-sm font-semibold tracking-wider group hover:bg-indigo-700 transition-colors duration-200">
								<div className="flex items-center gap-1 md:gap-2">
					...
					<tbody className="divide-y divide-slate-100">
						{isLoading ? (
							...
							filteredData.map((row: KelompokMkItem, idx: number) => (
								<tr
									key={row.id_kelompok_mk || idx}
								...
								>
									...
									<td className="px-3 md:px-6 py-3 md:py-4 text-right">
										<div className="inline-flex items-center gap-2">
											<Link
												href={`/siakad/admin/klpkmk/${row.id_kelompok_mk}`}
											...
											<button
												onClick={() => {
													if (confirm('Hapus kelompok mata kuliah ini?')) {
														console.log('delete', row.id_kelompok_mk);
